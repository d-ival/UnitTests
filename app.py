...
def check_str_value(value, input_name):
    if not value:
        value = input('Введите ' + input_name + ' - ')
...
def add_new_shelf(shelf_number=''):
    ...
    elif not isinstance(shelf_number, str):
...
def delete_doc(user_doc_number = ''):
    user_doc_number = check_str_value(user_doc_number, 'номер документа')
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
...
def get_doc_shelf(user_doc_number: str=None):
    if not isinstance(user_doc_number, str) or len(user_doc_number) == 0:
...
def add_new_doc(new_doc_number, new_doc_type, new_doc_owner_name, new_doc_shelf_number):
    ...
    new_doc_number = check_str_value(new_doc_number, 'номер документа')
    new_doc_type = check_str_value(new_doc_type, 'тип документа')
    new_doc_owner_name = check_str_value(new_doc_owner_name, 'имя владельца документа')
    new_doc_shelf_number = check_str_value(new_doc_shelf_number, 'номер полки для хранения')
...
if __name__ == '__main__':
    directories = update_directories()
